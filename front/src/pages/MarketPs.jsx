...
import { getAllps } from "../JS/Actions/PsActions";
import AddPs from "../components/AddPs";
import PsCard from "../components/psCard";

const MarketPs = () => {
  const load = useSelector((state) => state.PsReducer.load);
  const ps = useSelector((state) => state.PsReducer.ps || []);
    ...
    <div className="isolate bg-white px-6 py-16 sm:py-24 lg:px-8">
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-lg text-gray-600">
              Find premium consoles and accessories designed for a better gaming
              setup.
            </p>
          </div>
        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
          {ps && ps.map((psItem) => <PsCard ps={psItem} key={psItem._id} />)}
        </div>
      </div>
    </div>
  );
